...
with tabs[0]:
    ...
    if "generated_recipe" in st.session_state:
        ...
        if result.get("opportunities"):
            with st.expander("Developer Opportunity Debug", expanded=True):
                for opportunity in result["opportunities"]:
                    matched = " + ".join(opportunity.get("matched_resources") or [])
                    st.markdown(f"**✓ {opportunity.get('name', 'Culinary Opportunity')}**")
                    st.caption(
                        f"{opportunity.get('category', '').title()} · "
                        f"{opportunity.get('confidence', '')} confidence"
                        + (f" · {matched}" if matched else "")
                    )
                    st.write(opportunity.get("explanation", ""))
